...
private:
    ...
    // Helper to recursively delete the tree
...
public:
    void buildTree(string text) {
        ...
        for (char c : text) { // Use the class member frequencies map
            frequencies[c]++;
        }
    ...
    void printCodes() {
        ...
        for (auto& p : codes) {
            cout << "    " << p.first << "     |     "
                 << std::right <<  std::setw(4) << frequencies[p.first] << "  |  " << p.second << "\n";
        }
    } 
